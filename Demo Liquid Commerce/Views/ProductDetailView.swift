...
struct ProductDetailView: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                if let imagePath {
                    AsyncImage(url: URL(fileURLWithPath: imagePath)) { image in
                        image.resizable() } placeholder: { Image("image_placeholder").resizable() }
        ...
        .onAppear {
            Task {
                imagePath = try? await CachedAsyncImage(url: URL(string: product.images.first?.url ?? "")
                                                        ?? URL(fileURLWithPath: "")).imagePath
            }
            description = try? NSAttributedString(data: product.description.data(using: .unicode) ?? Data(),
