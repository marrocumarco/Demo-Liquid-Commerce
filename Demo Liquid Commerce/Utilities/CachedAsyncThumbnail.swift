...
public class CachedAsyncThumbnail: CachedAsyncImage {
    ...
    public override var imagePath: String {
        get async throws {
            _ = try await super.imagePath
            let pathExtension = fileURL.pathExtension
            let lastPathComponent = fileURL.deletingPathExtension().lastPathComponent.appending("_thumb")
            let thumbnailURL = fileURL.deletingLastPathComponent().appending(path: lastPathComponent,
            ...
            if !FileManager.default.fileExists(atPath: thumbnailURL.path()) {
                ...
                try await generator.saveBestRepresentation(for: request,
                                                           to: thumbnailURL,
